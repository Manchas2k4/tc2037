...
//				elements of an integer array using one thread and multiple
//        threads. Also, it calculates the SpeedUp.
//
...
int main(int argc, char* argv[]) {
	...
	blockCELLS = CELLS / THREADS;
	for (int i = 0; i < THREADS; i++) {
		blocks[i].arr = arr;
		blocks[i].start = i * blockCELLS;
		blocks[i].end = (i != (THREADS - 1))? (i + 1) * blockCELLS : CELLS;
		blocks[i].result = 0;
	}
